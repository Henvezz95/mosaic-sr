...
for i in range(4):
    ...
    for th in thresholds:
        ...
        if th_str not in results['psnr_mask']:
            results['psnr_mask'][th_str] = np.zeros((7,4)).tolist()
            results['psnr_all'][th_str] = np.zeros((7,4)).tolist()
            results['ssim_mask'][th_str] = np.zeros((7,4)).tolist()
            results['ssim_all'][th_str] = np.zeros((7,4)).tolist()
            results['zbar'][th_str] = np.zeros((7,4), dtype=np.int32).tolist()
        for j in tqdm(range(7)):
            ...
            for it in tqdm(iterator):
                ...
                for k in range(len(img_HD)):
                    img_up, _, _ = iterativeUpscale(img_LD[k,:,:,0], interpreters, np.power(th,2))
    ...
    with open('./results/results_iterative.yaml', 'w') as outfile:
