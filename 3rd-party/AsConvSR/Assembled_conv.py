...
class AssembledBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding=0,
        ...
        self.weight1 = nn.Parameter(torch.randn(E, out_channels, in_channels // groups, kernel_size, kernel_size).to(self.device), requires_grad=True)
        self.weight2 = nn.Parameter(torch.randn(E, out_channels, out_channels // groups, kernel_size, kernel_size).to(self.device), requires_grad=True)
        self.weight3 = nn.Parameter(torch.randn(E, out_channels, out_channels // groups, kernel_size, kernel_size).to(self.device), requires_grad=True)
        
        if self.bias:
            self.bias1 = nn.Parameter(torch.randn(E, out_channels).to(self.device), requires_grad=True) # E, out_channels
            self.bias2 = nn.Parameter(torch.randn(E, out_channels).to(self.device), requires_grad=True) # E, out_channels
            self.bias3 = nn.Parameter(torch.randn(E, out_channels).to(self.device), requires_grad=True) # E, out_channels
